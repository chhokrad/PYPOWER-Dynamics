...
class sym_order6b:
    def __init__(self, filename, dynopt):
        ...
        # Internal variables
        self.params['gamma_d1'] = (
            self.params['Xdpp'] - self.params['Xa']) / (self.params['Xdp'] - self.params['Xa'])
        self.params['gamma_d2'] = (
            1 - self.params['gamma_d1']) / (self.params['Xdp'] - self.params['Xa'])
        self.params['gamma_q1'] = (
            self.params['Xqpp'] - self.params['Xa']) / (self.params['Xqp'] - self.params['Xa'])
        self.params['gamma_q2'] = (
            1 - self.params['gamma_q1']) / (self.params['Xqp'] - self.params['Xa'])

        # Equivalent Norton impedance for Ybus modification
        self.Yg = (self.params['Ra'] - 1j * 0.5 * (self.params['Xdpp'] + self.params['Xqpp'])) / (
            self.params['Ra'] ** 2 + (self.params['Xdpp'] * self.params['Xqpp']))
    ...
    def initialise(self, vt0, S0):
        ...
        Vfd0 = Eqp0 + (Xd - Xdp) * (Id0 - gamma_d2 * phid_pp0 -
                                    (1 - gamma_d1) * Id0 + gamma_d2 * Eqp0)
    ...
    def check_diffs(self):
        ...
        dEqp = (Vfd - (Xd - Xdp) * (Id - gamma_d2 * phid_pp_0 -
                                    (1 - gamma_d1) * Id + gamma_d2 * Eqp_0) - Eqp_0) / Td0p
        dEdp = ((Xq - Xqp) * (Iq - gamma_q2 * phiq_pp_0 -
                              (1 - gamma_q1) * Iq - gamma_q2 * Edp_0) - Edp_0) / Tq0p
        ...
        if round(dEdp, 6) != 0 or round(dEqp, 6) != 0 or round(dphid_pp, 6) != 0 or round(dphiq_pp, 6) != 0:
            ...
            print('dEdp = ' + str(dEdp) + ', dEqp = ' + str(dEqp) +
                  ', dphid_pp = ' + str(dphid_pp) + ', dphiq_pp = ' + str(dphiq_pp))
    ...
    def calc_currents(self, vt):
        ...
        Id = (-Vq / omega + gamma_d1 * Eqp + (1 - gamma_d1) * phid_pp - Ra / (omega * Xqpp) *
              (Vd - gamma_q1 * Edp + (1 - gamma_q1) * phiq_pp)) / (Xdpp + Ra ** 2 / (omega * omega * Xqpp))
        Iq = (Vd / omega + (Ra * Id / omega) - gamma_q1 *
              Edp + (1 - gamma_q1) * phiq_pp) / Xqpp
    ...
    def solve_step(self, h, dstep):
        ...
        f1 = (Vfd - (Xd - Xdp) * (Id - gamma_d2 * phid_pp_0 -
                                  (1 - gamma_d1) * Id + gamma_d2 * Eqp_0) - Eqp_0) / Td0p
        k_Eqp = h * f1

        f2 = ((Xq - Xqp) * (Iq - gamma_q2 * phiq_pp_0 -
                            (1 - gamma_q1) * Iq - gamma_q2 * Edp_0) - Edp_0) / Tq0p
        k_Edp = h * f2
        ...
        # Swing equation
        f5 = 0.5 / self.params['H'] * \
            (self.signals['Pm'] / omega_0 - self.signals['P'])
        k_omega = h * f5
        ...
        if self.opt == 'mod_euler':
            ...
            if dstep == 0:
                ...
                # Corrector step
                self.states['Eqp'] = Eqp_0 + 0.5 * \
                    (k_Eqp - self.dsteps['Eqp'][0])
                self.states['Edp'] = Edp_0 + 0.5 * \
                    (k_Edp - self.dsteps['Edp'][0])
                self.states['phiq_pp'] = phiq_pp_0 + 0.5 * \
                    (k_phiq_pp - self.dsteps['phiq_pp'][0])
                self.states['phid_pp'] = phid_pp_0 + 0.5 * \
                    (k_phid_pp - self.dsteps['phid_pp'][0])
                self.states['omega'] = omega_0 + 0.5 * \
                    (k_omega - self.dsteps['omega'][0])
                self.states['delta'] = delta_0 + 0.5 * \
                    (k_delta - self.dsteps['delta'][0])
            ...
            if dstep == 0:
                ...
                self.states['Eqp'] = self.states0['Eqp'] + 1/6 * (
                    self.dsteps['Eqp'][0] + 2*self.dsteps['Eqp'][1] + 2*self.dsteps['Eqp'][2] + k_Eqp)
                self.states['Edp'] = self.states0['Edp'] + 1/6 * (
                    self.dsteps['Edp'][0] + 2*self.dsteps['Edp'][1] + 2*self.dsteps['Edp'][2] + k_Edp)
                self.states['phiq_pp'] = self.states0['phiq_pp'] + 1/6 * (
                    self.dsteps['phiq_pp'][0] + 2*self.dsteps['phiq_pp'][1] + 2*self.dsteps['phiq_pp'][2] + k_phiq_pp)
                self.states['phid_pp'] = self.states0['phid_pp'] + 1/6 * (
                    self.dsteps['phid_pp'][0] + 2*self.dsteps['phid_pp'][1] + 2*self.dsteps['phid_pp'][2] + k_phid_pp)
                self.states['omega'] = self.states0['omega'] + 1/6 * (
                    self.dsteps['omega'][0] + 2*self.dsteps['omega'][1] + 2*self.dsteps['omega'][2] + k_omega)
                self.states['delta'] = self.states0['delta'] + 1/6 * (
                    self.dsteps['delta'][0] + 2*self.dsteps['delta'][1] + 2*self.dsteps['delta'][2] + k_delta)
                self.signals['Tm'] = self.signals['Pm'] / omega_0

        ...
        for line in self.equations:
            ...
            if block == 'CONST':
                ...
                if function not in self.ext_functions.keys():
                    ...
                    func = getattr(module, func_name[1])
                    self.ext_functions[function] = func
                self.signals = self.ext_functions[function](
                    self.id, self.signals)
                yo = None
            ...
            if x1:
                if self.opt == 'mod_euler':
                    ...
                    if dstep == 0:
                        ...
                        self.states[signal] = self.states0[signal] + 1/6 * (
                            self.dsteps[signal][0] + 2*self.dsteps[signal][1] + 2*self.dsteps[signal][2] + f * h)
